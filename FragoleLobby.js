...
function callAll(func, args) {
    ...
    for(let player of gameController.players.iterator()) {
      if(player[ITEM].session) {
        player[ITEM].session[func](...args);
      }
    }
}
...
class FragoleLobby extends EventEmitter {
  constructor () {
    super();
    gameController.rpcServer.connect('playerReady', this.playerReady, this);
    this.playersReady = {};
